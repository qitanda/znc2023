...
def detection_img(results, image):
    ...
    for res in results:
        ...
        centre.append((int(left)+int(right))/2, (int(top)+int(bottom)/2))
        color = (0, 244, 10)
...
def camera_task(detector):
    ...
    while True:
        ...
        sys.stdout.flush()
        if len(res) > 0:
            image,center,name = detection_img(res, image)
            print(center)
            sys.stdout.flush()
        cv2.imshow("test", image)
        print(center)
        sys.stdout.flush()
        # cv2.circle(image, image.point, 1, (0, 0, 255), 2)
        cv2.waitKey(1)
